...
function markSvg(providerId: ProviderLimitId): SVGSVGElement {
...
export function renderUsageMeters(
  ...
  for (const provider of providers) {
    ...
    figures.className = "pcts";
    const windows =
      provider.windows.length > 0 ? provider.windows : [null];
    for (const window of windows) {
      const remaining = window?.remainingPercent ?? null;
      const figure = document.createElement("span");
      figure.className = `pct pct-${limitTone(remaining)}`;
      figure.textContent = remaining === null ? "–" : `${remaining}%`;
